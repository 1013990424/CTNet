...
class TrainData(Dataset):
    def __init__(self, size=384, loader=loader,  transform=None):
        ...
        img = os.listdir(self.path + 'Synthetic/Train/Normal')
        data = []
        for name in img:
            data.append(
                [self.path + 'Synthetic/Train/Low/' + name, self.path + 'Synthetic/Train/Normal/' + name])
        # 打乱数据集
        random.shuffle(data)
...
class TestData(Dataset):
    def __init__(self, size=384, loader=loader,  transform=None):
        ...
        # 读取全部数据集
        self.path = '/data/low_light_dataset/LOL-v2/'
        img = os.listdir(self.path + 'Synthetic/Test/Normal')
        data = []
        for name in img:
            data.append(
                [self.path + 'Synthetic/Test/Low/' + name, self.path + 'Synthetic/Test/Normal/' + name])
        self.transform = transform
        self.size = size
        self.loader = loader
        self.data = data
