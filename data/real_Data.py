...
class TrainData(Dataset):
    def __init__(self, size=384, loader=loader,  transform=None):
        ...
        # 读取全部数据集
        self.path = '/data/low_light_dataset/LOL-v2/'
        img = os.listdir(self.path + 'Real_captured/Train/Normal')
        data = []
        for name in img:
            name = name.split('normal')[1]
            data.append(
                [self.path + 'Real_captured/Train/Low/low' + name, self.path + 'Real_captured/Train/Normal/normal' + name])
        # 打乱数据集
        random.shuffle(data)
...
class TestData(Dataset):
    def __init__(self, size=384, loader=loader,  transform=None):
        ...
        # 读取全部数据集
        self.path = '/data/low_light_dataset/LOL-v2/'
        img = os.listdir(self.path + 'Real_captured/Test/Normal')
        data = []
        for name in img:
            name = name.split('normal')[1]
            data.append(
                [self.path + 'Real_captured/Test/Low/low' + name, self.path + 'Real_captured/Test/Normal/normal' + name])
        self.transform = transform
        self.size = size
        self.loader = loader
        self.data = data
