...
def eval_images(enhanced_root, gt_root, TEST_PREFIXES):
    ...
    for folder_name in os.listdir(enhanced_root):
        ...
        assert len(enhanced_list) == len(gt_list), f"图像数量不一致：{folder_name}"

        for enh_name, gt_name in zip(enhanced_list, gt_list):
...
if SDSD_type == 'indoor':
    ...
    TEST_PREFIXES = ['MVI_1003', 'MVI_0975_2', 'MVI_0997', 'MVI_1026_2',
                     ...
                     'MVI_0997_2', 'MVI_1030', 'MVI_1003_2', 'MVI_0975', 'MVI_1 032',
                     'MVI_1001_2']

# 设置增强结果路径和 GT 图像路径（均为 PNG / JPG 格式图像）
enhanced_path = '/home/pc/Desktop/SDSD/' +SDSD_type + '/CTNet'
