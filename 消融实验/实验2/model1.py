...
class Block(nn.Module):
    def __init__(self, input_dim):
        ...
        self.conv = nn.Conv2d(input_dim * 3, input_dim, 1, 1, 0)
        self.lrelu = nn.LeakyReLU()
        self.sigmoid = nn.Sigmoid()

    def forward(self, hfea, sfea, vfea):
        fea = torch.cat([hfea, sfea, vfea], dim=1)
        fea = self.lrelu(self.conv(fea))
...
class MCFNet(nn.Module):
    ...
    def forward(self, x):
        ...
        fea = torch.cat([fea, rgb_fea3, hsv_fea3], dim=1)# 128 + 64
        fea = self.at1(fea)
        ...
        fea = torch.cat([fea, rgb_fea2, hsv_fea2], dim=1)# 64 + 32
        fea = self.at2(fea)
        ...
        fea = torch.cat([fea, rgb_fea1, hsv_fea1], dim=1)# 32 + 16
        fea = self.at3(fea)
